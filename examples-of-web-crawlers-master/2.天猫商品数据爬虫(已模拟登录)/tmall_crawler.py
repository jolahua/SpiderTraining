...
from tools import chromedriver_path, weibo_password, weibo_username
...
class taobao_infos:
    ...
    def login(self):
        ...
        tn_css = '.site-nav-bd > ul.site-nav-bd-l > li#J_SiteNavLogin > div.site-nav-menu-hd' \
                 ' > div.site-nav-user > a.site-nav-login-info-nick '
        taobao_name = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, tn_css)))
        # 输出淘宝昵称
    ...
    def next_page(self, page_number):
        # 等待该页面input输入框加载完毕
        input_css = '.ui-page > div.ui-page-wrap > b.ui-page-skip > form > input.ui-page-skipTo'
        input_button = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, input_css)))
        # 等待该页面的确定按钮加载完毕
        submit_css = '.ui-page > div.ui-page-wrap > b.ui-page-skip > form > button.ui-btn-s'
        submit = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, submit_css)))
        # 清除里面的数字
        input_button.clear()
        # 重新输入数字
    ...
    def crawl_good_data(self):
        ...
        for page in range(2, int(page_total)):
            # 等待该页面全部商品数据加载完毕
            good_total_css = '#J_ItemList > div.product > div.product-iWrap'
            good_total = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, good_total_css)))
            # 等待该页面input输入框加载完毕
            input_css = '.ui-page > div.ui-page-wrap > b.ui-page-skip > form > input.ui-page-skipTo'
            input_button = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, input_css)))
            # 获取当前页
            now_page = input_button.get_attribute('value')
